...
class Books::Leaves::MovesControllerTest < ActionDispatch::IntegrationTest
  test "moving a single item" do
    ...
    post book_leaves_moves_url(books(:handbook), id: leaves(:welcome_page), position: 0)
  ...
  test "moving multiple items" do
    ...
    post book_leaves_moves_url(books(:handbook), id: leaves(:summary_page, :reading_picture), position: 1)
    assert_response :no_content
